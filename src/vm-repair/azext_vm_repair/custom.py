...
def create(cmd, vm_name, resource_group_name, repair_password=None, repair_username=None, repair_vm_name=None, copy_disk_name=None, repair_group_name=None, unlock_encrypted_vm=False, enable_nested=False, associate_public_ip=False, distro='ubuntu', yes=False, encrypt_recovery_key="", disable_trusted_launch=False, os_disk_type=None, tags=None):  
    ...
    - tags: Tags to apply to the repair VM. 
        - CLI usage: pass as a string in Azure CLI standard format (e.g. 'key1=value1 key2=value2').
        - Internal usage: may be a dict or a string. If a dict, it will be converted to CLI format.

    Note: The Azure CLI only supports the string format for tags (key[=value] [key[=value] ...]).
    Internal code may use either a dict or a string for flexibility, but CLI users must use the string format.
    """  
    ...
    try:  
        ...
        if is_linux:
            create_repair_vm_command = (
                f'az vm create -g {repair_group_name} -n {repair_vm_name} --image {os_image_urn} '
